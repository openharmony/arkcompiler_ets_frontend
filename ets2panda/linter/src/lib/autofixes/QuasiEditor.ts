...
const DEPENDENT_RULES_IN_AUTOFIX: Set<number> = new Set([
    faultsAttrs[FaultID.ObjectLiteralNoContextType].cookBookRef
]);
...
export class QuasiEditor {
    ...
    private saveText(): void {
        ...
        let srcLines = this.textBuffer.split(EOL);
        for ( let str of srcLines ) {
    ...
    private replaceAt(start: number, len: number, newText: string): void {
        let head = this.textBuffer.slice(0, start);
        let tail = this.textBuffer.slice(start + len);
    ...
    public applyFixes(problemInfos: ProblemInfo[]): void {
        ...
        for (let i = problemInfos.length - 1; i >= 0; i--) {
            ...
            for (let j = pInfo.autofix.length - 1; j >= 0; j--) {
                ...
                this.replaceAt(lastFixStart = pInfo.autofix[j].start,
