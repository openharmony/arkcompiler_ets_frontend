...
export class LoopBuilder {
    public rebuildBlocksInLoop(blockBuilderToCfgBlock: Map<BlockBuilder, BasicBlock>,
                               blocksContainLoopCondition: Set<BlockBuilder>,
                               basicBlockSet: Set<BasicBlock>, blockBuilders: BlockBuilder[]): void {
            ...
            if (iteratorNextStmtIdx !== -1 || dummyInitializerStmtIdx !== -1) {
                ...
                if (prevBlockBuilderContainsLoop) {
                    ...
                    this.insertBeforeConditionBlockBuilder(blockBuilderToCfgBlock, blockBuilder,
                        stmtsInsertBeforeCondition, false, basicBlockSet, blockBuilders);
                } else {
                ...
                if (dummyInitializerStmtIdx !== -1 && ifStmtIdx !== stmtsCnt - 1) {
                    ...
                    this.adjustIncrementorStmts(stmts, ifStmtIdx, blockBuilder, blockId, blockBuilderToCfgBlock,
                        blocksContainLoopCondition, basicBlockSet, emptyLoopBody, blockBuilders);
    ...
    private doesPrevBlockBuilderContainLoop(currBlockBuilder: BlockBuilder, currBlockId: number,
                                            blocksContainLoopCondition: Set<BlockBuilder>): boolean {
    ...
    private insertBeforeConditionBlockBuilder(
        ...
        const {
            collectedBlockBuilders, collectedBlocks,
        } = this.getCollectedBlocks(collectReenter, blockBuildersBeforeCondition, blocksBeforeCondition,
            blockBuildersReenterCondition, blocksReenterCondition);

        const { blockBuilderInsertBeforeCondition, blockInsertBeforeCondition } = this.createAndLinkBlocks(
            collectedBlockBuilders, collectedBlocks, conditionBlockBuilder, stmtsInsertBeforeCondition, block);

        this.updatePredecessors(collectedBlockBuilders, blockBuilderToCfgBlock, conditionBlockBuilder,
            blockBuilderInsertBeforeCondition, blockInsertBeforeCondition);

        const { newPrevBlockBuildersBeforeCondition, newPrevBlocksBeforeCondition } = this.getNewPrevBlocks(
            collectReenter, blockBuildersBeforeCondition, blocksBeforeCondition, blockBuilderInsertBeforeCondition,
            blockInsertBeforeCondition, blockBuildersReenterCondition, blocksReenterCondition);
    ...
    private getBlockFromMap(blockBuilderToCfgBlock: Map<BlockBuilder, BasicBlock>,
                            conditionBlockBuilder: BlockBuilder): BasicBlock {
    ...
    private collectBlocksBeforeAndReenter(
        ...
        blockBuildersBeforeCondition: BlockBuilder[], blocksBeforeCondition: BasicBlock[],
        blockBuildersReenterCondition: BlockBuilder[], blocksReenterCondition: BasicBlock[]
    } {
    ...
    private createAndLinkBlocks(
        ...
        stmtsInsertBeforeCondition: Stmt[],
        block: BasicBlock,
    ): { blockBuilderInsertBeforeCondition: BlockBuilder, blockInsertBeforeCondition: BasicBlock } {
    ...
    private getNewPrevBlocks(
        ...
        blockBuildersReenterCondition: BlockBuilder[],
        blocksReenterCondition: BasicBlock[],
    ): { newPrevBlockBuildersBeforeCondition: BlockBuilder[], newPrevBlocksBeforeCondition: BasicBlock[] } {
    ...
    private adjustIncrementorStmts(stmts: Stmt[], ifStmtIdx: number, currBlockBuilder: BlockBuilder,
                                   currBlockId: number, blockBuilderToCfgBlock: Map<BlockBuilder, BasicBlock>,
                                   blocksContainLoopCondition: Set<BlockBuilder>, basicBlockSet: Set<BasicBlock>,
                                   emptyLoopBody: boolean, blockBuilders: BlockBuilder[]): void {
        ...
        if (blockBuildersReenterCondition.length > 1 || blocksContainLoopCondition.has(
            ...
            this.insertBeforeConditionBlockBuilder(blockBuilderToCfgBlock, currBlockBuilder,
                stmtsReenterCondition, true, basicBlockSet, blockBuilders);
        } else {
