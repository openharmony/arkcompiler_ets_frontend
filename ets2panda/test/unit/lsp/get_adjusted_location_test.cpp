...
let z = x;
    ...
    auto adjustedVar = ark::es2panda::lsp::GetAdjustedLocation(varNode);
...
let instance = new MyClass();
    ...
    es2panda_Context *ctx = initializer.CreateContext("test.ets", ES2PANDA_STATE_CHECKED, source);
    ASSERT_EQ(ContextState(ctx), ES2PANDA_STATE_CHECKED);
    AstNode *classNode = ark::es2panda::lsp::GetTouchingToken(ctx, CLASS_POS, false);
    ASSERT_NE(classNode, nullptr);
    ASSERT_TRUE(classNode->IsClassDeclaration());
    auto adjustedClass = ark::es2panda::lsp::GetAdjustedLocationForClass(classNode);
...
let result = testFunc(21);
    ...
    auto adjustedFunc = ark::es2panda::lsp::GetAdjustedLocationForFunction(parent);
...
class Test {
    ...
    ASSERT_NE(classNode, nullptr);
    ASSERT_TRUE(classNode->IsClassDeclaration());
    auto children = ark::es2panda::lsp::GetChildren(classNode);
...
TEST_F(LspGetAdjustedLocation, GetAdjustedLocationForNestedScopeTest)
{
    ...
    if (globalVar != nullptr) {
        auto adjustedGlobal = ark::es2panda::lsp::GetAdjustedLocation(globalVar);
    ...
    if (memberVar != nullptr) {
        auto adjustedMember = ark::es2panda::lsp::GetAdjustedLocation(memberVar);
...
let instance = new MyClass();
    ...
    ASSERT_NE(node, nullptr);
    auto adjusted = ark::es2panda::lsp::GetAdjustedLocation(node);
...
let numbers: NumArray;
    ...
    ASSERT_NE(node, nullptr);
    auto children = ark::es2panda::lsp::GetChildren(node);
...
class TestClass {
    ...
    auto methodChildren = ark::es2panda::lsp::GetChildren(methodBody);
