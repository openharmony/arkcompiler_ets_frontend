...
// Test: barrel file `export { A } from './a';` should be usable as an import source
TEST_F(FixImportSourceTest1, ImportThroughBarrelFile)
{
    std::vector<std::string> fileNames = {"origin.ets", "barrel.ets", "barrel_consumer.ets"};
    std::vector<std::string> fileContents = {
        R"(export class Widget {
    size: number = 0;
})",
        R"(export { Widget } from './origin';)",
    ...
    // Index both the origin and barrel files so the fix can discover them
    {
    ...
    const auto unresolvedPos = fileContents[2].find("Widget()");
    ...
    // The fix should add an import statement referencing Widget
    ASSERT_NE(updated.find("import"), std::string::npos);
    ASSERT_NE(updated.find("Widget"), std::string::npos);

    initializer.DestroyContext(context);
}

// Test: when multiple files export the same symbol name, the fix should pick a valid source
TEST_F(FixImportSourceTest1, MultipleSourcesWithSameExportName)
{
    std::vector<std::string> fileNames = {"source_a.ets", "source_b.ets", "multi_consumer.ets"};
    std::vector<std::string> fileContents = {
        R"(export class Item {
    id: number = 0;
})",
    ...
    // Index both source files
    {
        Initializer indexInitializer;
        auto *ctxA = indexInitializer.CreateContext(filePaths[0].c_str(), ES2PANDA_STATE_CHECKED);
        ark::es2panda::lsp::BuildSymbolReferenceIndexForContext(ctxA);
        indexInitializer.DestroyContext(ctxA);
    }
    {
        Initializer indexInitializer;
        auto *ctxB = indexInitializer.CreateContext(filePaths[1].c_str(), ES2PANDA_STATE_CHECKED);
        ark::es2panda::lsp::BuildSymbolReferenceIndexForContext(ctxB);
        indexInitializer.DestroyContext(ctxB);
    }

    Initializer initializer;
    auto *context = initializer.CreateContext(filePaths[2].c_str(), ES2PANDA_STATE_CHECKED);

    const auto unresolvedPos = fileContents[2].find("Item()");
    ASSERT_NE(unresolvedPos, std::string::npos);
    ...
    const auto updated = ApplyFirstChange(fileContents[2], importSourceFix.value());
    // The fix should add an import referencing Item from one of the sources
    ASSERT_NE(updated.find("import"), std::string::npos);
    ASSERT_NE(updated.find("Item"), std::string::npos);
