...
class LspGetNodeEtsStructDeclarationTests : public LSPAPITests {
protected:
...
struct Person {
    ...
    {
        contexts_ =
            initializer.CreateContext("GetNodeStructDeclarationTest.ets", ES2PANDA_STATE_PARSED, sourceCode_.c_str());
    }
...
TEST_F(LspGetNodeEtsStructDeclarationTests, GetEtsStructDeclarationNonExistentTest)
{
    LSPAPI const *lspApi = GetImpl();
    const std::string nodeName = "NonExistentStruct";
    std::vector<NodeInfo> nodeInfos;
    nodeInfos.emplace_back(NodeInfo {nodeName, ark::es2panda::ir::AstNodeType::STRUCT_DECLARATION});
...
TEST_F(LspGetNodeEtsStructDeclarationTests, GetEtsStructDeclarationTest1)
{
    LSPAPI const *lspApi = GetImpl();
    const std::string nodeName = "Index";
    std::vector<NodeInfo> nodeInfos;
    nodeInfos.emplace_back(NodeInfo {nodeName, ark::es2panda::ir::AstNodeType::STRUCT_DECLARATION});
    std::vector<NodeInfo *> nodeInfoPtrs;
    nodeInfoPtrs.push_back(&nodeInfos[0]);
...
TEST_F(LspGetNodeEtsStructDeclarationTests, GetStructDeclarationTest2)
{
    LSPAPI const *lspApi = GetImpl();
    const std::string nodeName = "Person";
    std::vector<NodeInfo> nodeInfos;
    nodeInfos.emplace_back(NodeInfo {nodeName, ark::es2panda::ir::AstNodeType::STRUCT_DECLARATION});
    std::vector<NodeInfo *> nodeInfoPtrs;
    nodeInfoPtrs.push_back(&nodeInfos[0]);
