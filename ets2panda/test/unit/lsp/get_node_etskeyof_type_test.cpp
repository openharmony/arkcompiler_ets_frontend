...
class LspGetNodeKeyofTypeTests : public LSPAPITests {
protected:
...
type UserKeys = keyof User;
    ...
    {
        contexts_ = initializer.CreateContext("GetNodeKeyofType.ets", ES2PANDA_STATE_PARSED, sourceCode_.c_str());
    }
...
TEST_F(LspGetNodeKeyofTypeTests, GetKeyOfTypeNonExistentTest)
{
    LSPAPI const *lspApi = GetImpl();
    const std::string moduleName = "NonExistent";
    std::vector<NodeInfo> nodeInfos;
    nodeInfos.emplace_back(NodeInfo {moduleName, ark::es2panda::ir::AstNodeType::ETS_KEYOF_TYPE});
...
TEST_F(LspGetNodeKeyofTypeTests, GetKeyOfTypeTest1)
{
    LSPAPI const *lspApi = GetImpl();
...
TEST_F(LspGetNodeKeyofTypeTests, GetKeyOfTypeTest2)
{
    LSPAPI const *lspApi = GetImpl();
