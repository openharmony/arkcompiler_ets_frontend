...
public:
    ...
    void CheckFunctionAnnotations(pandasm::Program *program)
    {
        ...
        const AnnotationMap expectedFuncAnnotations = {
            {"ClassAuthor",
             {
                 {"mutiArray", "foo$ClassAuthor$mutiArray$19"},
                 {"color", "foo$ClassAuthor$color$20"},
                 {"testBools", "foo$ClassAuthor$testBools$21"},
                 {"reviewers", "foo$ClassAuthor$reviewers$22"},
                 {"favorColor", "1"},
                 {"testBool", "0"},
                 {"reviewersAge", "foo$ClassAuthor$reviewersAge$23"},
                 {"authorAge", "35.000000"},
                 {"authorName", "Jim"},
             }},
        };
    ...
    void CheckLiteralArrayTable(pandasm::Program *program)
    {
        std::vector<std::pair<std::string, std::vector<AnnotationValueType>>> expectedLiteralArrayTable = {
            ...
            {"foo$ClassAuthor$color$20", std::vector<AnnotationValueType> {COLOR_OPTION_0, COLOR_OPTION_1}},
            {"foo$ClassAuthor$reviewers$22",
             std::vector<AnnotationValueType> {std::string("Bob"), std::string("Jim"), std::string("Tom")}},
            {"foo$ClassAuthor$reviewersAge$23", std::vector<AnnotationValueType> {AGE_18, AGE_21, AGE_32}},
            {"foo$ClassAuthor$testBools$21", std::vector<AnnotationValueType> {false, true, false}},
            {"foo$ClassAuthor$mutiArray$16", std::vector<AnnotationValueType> {VALUE_1, VALUE_2, VALUE_3}},
            {"foo$ClassAuthor$mutiArray$17", std::vector<AnnotationValueType> {VALUE_4, VALUE_5, VALUE_6}},
            {"foo$ClassAuthor$mutiArray$18", std::vector<AnnotationValueType> {VALUE_7, VALUE_8, VALUE_9}},
            {"foo$ClassAuthor$mutiArray$19",
             std::vector<AnnotationValueType> {std::string("foo$ClassAuthor$mutiArray$16"),
                                               std::string("foo$ClassAuthor$mutiArray$17"),
                                               std::string("foo$ClassAuthor$mutiArray$18")}},
        };
