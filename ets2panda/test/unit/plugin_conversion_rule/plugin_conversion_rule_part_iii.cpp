...
// apiName: CreateETSScript
TEST_F(PluginConversionRuleUnitTest, ParserProgramPtrInputParameter)
{
    std::string targetCAPI {R"(
    extern "C" es2panda_AstNode *CreateETSScript([[maybe_unused]] es2panda_Context *context,
    [[maybe_unused]] es2panda_AstNode **statementList, size_t statementListLen)
    {
        ...
        ArenaVector<ir::Statement *> statementListArenaVector {reinterpret_cast<Context *>(context)
        ->allocator->Adapter()};
        ...
        return reinterpret_cast<es2panda_AstNode *>(ctxAllocator->New<ir::ETSScript>(allocatorE2p,
               std::move(statementListArenaVector), programE2p));
    })"};
