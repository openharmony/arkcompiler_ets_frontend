...
static void FindFooDef(es2panda_AstNode *ast)
{
    if (!impl->IsMethodDefinition(ast)) {
        impl->AstNodeIterateConst(ctx, ast, FindFooDef);
        return;
    }
    auto scriptFunc = impl->MethodDefinitionFunction(ctx, ast);
    if (scriptFunc == nullptr ||
        std::string("foo") != impl->IdentifierName(ctx, impl->ScriptFunctionId(ctx, scriptFunc))) {
        impl->AstNodeIterateConst(ctx, ast, FindFooDef);
        return;
    }
    fooScrFunc = scriptFunc;
}
...
int main(int argc, char **argv)
{
    ...
    if (fooScrFunc == nullptr) {
        return TEST_ERROR_CODE;
    }
    ASSERT(impl->IsScriptFunction(fooScrFunc));
    auto flags = impl->ScriptFunctionFlagsConst(context, fooScrFunc);
