...
static int CheckAssignmentExpression(es2panda_AstNode *programNode)
{
    ...
    if (!impl->IsAssignmentExpression(assignmentExpr)) {
...
static int CheckCreateBinaryExpression(es2panda_AstNode *programNode)
{
    es2panda_AstNode *identifier = CreateIdentifierFromString(context, "x");
    ...
    es2panda_AstNode *assignmentStat = impl->CreateExpressionStatement(context, assignmentExpr);
    if (!impl->IsExpressionStatement(assignmentStat)) {
        return TEST_ERROR_CODE;
    }
    impl->AstNodeSetParent(context, identifier, assignmentExpr);
    impl->AstNodeSetParent(context, numberLiteral, assignmentExpr);
...
int main(int argc, char **argv)
{
    ...
    code = CheckAssignmentExpression(programNode);
    if (code != 0) {
        std::cerr << "FAILED TO CHECK ASSIGNMENT EXPRESSION" << std::endl;
        return code;
    }
    code = CheckCreateBinaryExpression(programNode);
    if (code != 0) {
        std::cerr << "FAILED TO CHECK CREATE BINARY EXPRESSION" << std::endl;
        return code;
    }
