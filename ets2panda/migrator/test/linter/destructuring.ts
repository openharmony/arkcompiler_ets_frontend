...
function destructuringParameters(): void {
    ...
    const hello = ({firstName, lastName}: {firstName: string, lastName: string}): string => `Hello ${firstName} ${lastName}`;
...
function destructuringAssignments(): void {
    ...
    let x: {e: number};
    ({ a, b: { c, d: [e, f]} } = {a: 10, b: { c: "foo", d: [30, 40]}});
    [a, b, [x, { f }]] = [1, 2, [{e: 20}, {f: 5}]];
}

function destructuringDeclarations(): void {
    let {q, w, e} = {q: 1, w: "foo", e: true};

    function getSomeObject() {
        return {x: 1, y: 2}
    }
    let {x, y} = getSomeObject();

    let [i, j, k] = [10, 20, 30];

    let getArray = (): number[] => [1, 2, 3];
    let [a, b] = getArray();
}

function loopVariables(): void {
    let objects: { a: number, b: string }[] = [
        {a: 10, b: "q"},
        {a: 20, b: "w"},
        {a: 30, b: "e"}
    ];
    ...
    const people = [
        { name: "Mike Smith", family: { mother: "Jane Smith", father: "Harry Smith", } },
        { name: "Tom Jones", family: { mother: "Norah Jones", father: "Richard Jones", } },
    ];
