...
function parseJson5(filePath: string) {
    try {
        const rawContent = fs.readFileSync(filePath, 'utf8');
        const cleanedContent = removeTrailingCommas(rawContent);
        return JSON5.parse(cleanedContent);
    } catch (error) {
...
function getEtsFiles(modulePath: string): string[] {
    ...
    const processEntry = (dir: string, entry: fs.Dirent) => {
        const fullPath = path.join(dir, entry.name);
        const relativePath = path.relative(modulePath, fullPath);

        if (entry.isDirectory()) {
            if (shouldSkipDirectory(relativePath)) return;
...
export function generateBuildConfigs(buildSdkPath: string, projectRoot: string, modules?: ModuleDescriptor[]): Record<string, any> {
    const allBuildConfigs: Record<string, any> = {};
