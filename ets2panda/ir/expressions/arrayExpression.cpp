                                                       ...
                                                       checker::TypeRelationFlag flags)
{
    ...
    if (param->IsETSUnionType()) {
        // Defer the union disambiguation to the array literal check: ETSAnalyzer reports
        // AMBIGUOUS_ARRAY_LITERAL_TYPE when the literal (empty or not) matches several union constituents.
        SetPreferredType(param);
        return;
    }
