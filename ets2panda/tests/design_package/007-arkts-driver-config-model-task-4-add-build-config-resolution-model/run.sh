...
TASK_ID="007-arkts_driver_config_model-task-4-add-build-config-resolution-model"
...
UNSUPPORTED_CONFIG="$REPO_ROOT/driver/build_system/test/ets_ut/unsupported_mode_build_config.json"
TIMEOUT_SEC=45
...
cfg = {
    ...
    "compilerOptions": {
        "package": "@val_process_config_baseline",
        "baseUrl": str(pkg_dir),
        "rootDir": str(pkg_dir),
...
require_file "$PROD_SRC/init/process_build_config.ets" "production process_build_config source not found"
...
require_file "$PROCESS_TEST" "process build config test source not found"
require_file "$UNSUPPORTED_CONFIG" "unsupported mode fixture not found"
...
python3 - "$PROD_SRC" "$DEMO_CONFIG" "$UNSUPPORTED_CONFIG" "$ARTIFACT_DIR/source_and_fixture_assertion.txt" <<'PY'
import json
import pathlib
import re
import sys
...
unsupported = pathlib.Path(sys.argv[3])
out = pathlib.Path(sys.argv[4])
for path in (demo, unsupported):
    data = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(data, dict):
        out.write_text(f"FAIL {path} is not a JSON object\n", encoding="utf-8")
        sys.exit(1)
unsupported_data = json.loads(unsupported.read_text(encoding="utf-8"))
if unsupported_data.get("es2pandaMode") == "sequential":
    out.write_text("FAIL unsupported-mode fixture is sequential\n", encoding="utf-8")
    sys.exit(1)
for path in src.rglob("*.ets"):
    ...
    if re.search(r"^\s*import\s+.*\b(child_process|fs|path|os|process)\b", text, re.MULTILINE):
        ...
        sys.exit(1)
out.write_text("PASS fixtures valid and no forbidden Node imports in ets_src\n", encoding="utf-8")
PY
[ $? -eq 0 ] || fail "source/fixture assertion failed; see $ARTIFACT_DIR/source_and_fixture_assertion.txt"
...
process_status=$?
[ $process_status -eq 0 ] || fail "process build config validation runtime failed with exit $process_status; see $ARTIFACT_DIR/process_runtime.stderr.log"
...
[ $? -eq 0 ] || fail "process build config assertions failed; see $ARTIFACT_DIR/process_stdout_assertion.txt"
