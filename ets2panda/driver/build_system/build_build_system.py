...
def build(options):
    ...
    # Copy .node files if they exist (generated by GN build)
    public_node = os.path.join(bindings_dir, 'public.node')
    if os.path.exists(public_node):
        copy_files(public_node, os.path.join(bindings_out, 'public.node'), True)
