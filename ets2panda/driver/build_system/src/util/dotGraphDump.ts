...
import { JobInfo, JobContentType, FileInfo } from '../types';

function formLabelForNode(node: GraphNode<JobInfo>): string {
    const job = node.data;
    let res: string = `{ id: ${node.id.slice(0, 5)}`
    if (node.data.contentType === JobContentType.CLUSTER) {
        for (const fi of (job.content as FileInfo[])) {
            res += ` | ${path.join(job.moduleName, path.relative(job.moduleRoot, fi.input))}`
        }
    } else {
        res += ` | ${path.join(job.moduleName, path.relative(job.moduleRoot, (job.content as FileInfo).input))}`
    }
    res += '}'
    return res;
}
...
function createNodes(dorGraph: dot.Digraph, graph: Graph<JobInfo>): void {
    for (const node of graph.nodes) {
        const dotNode = new dot.Node(node.id, {
            ...
            [dot.attribute.fillcolor]: node.data.contentType === JobContentType.CLUSTER ? 'lightcoral' : undefined
        })
