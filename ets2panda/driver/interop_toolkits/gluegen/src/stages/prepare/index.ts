...
export function createPrepareStage() {
  return Stage.start<GlueGenContext>('prepare')
    ...
    .use('generate-interop-file-list', {
      inputs: [],
      run: generateInteropFileList,
    })
    .provides(INTEROP_FILE_LIST_ARTIFACT, {
      build: (_scope, outputs) => outputs['generate-interop-file-list'],
    });
}

export type { InteropFileListPath } from './generateInteropFileList';
