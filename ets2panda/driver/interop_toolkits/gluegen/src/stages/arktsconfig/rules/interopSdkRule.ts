...
/** Generates dependencies contributed by the dynamic interoperability SDK. */
export class InteropSdkRule implements ArkTSConfigRule {
  async generate(context: GenerationContext): Promise<RuleOutput> {
    ...
    for (const sdkPath of await this.interopSdkPaths(context)) {
      ...
      for (const filePath of await collectFiles(sdkPath, (candidate) => candidate.endsWith(ETS_DECLARATION_SUFFIX))) {
        const [key, item] = this.createDependency(sdkPath, filePath, prefix);
        output.dependencies.set(key, item);
      }
    }
    return output;
  }

  private async interopSdkPaths(context: GenerationContext): Promise<readonly string[]> {
    const { buildConfig, moduleTable } = context;
    const basePaths =
      buildConfig.interopApiPaths.length === 0
        ? [path.resolve(buildConfig.buildSdkPath, '..', 'dynamic', 'build-tools', 'interop')]
        : buildConfig.interopApiPaths.map((basePath) =>
            path.isAbsolute(basePath) ? basePath : path.resolve(moduleTable.mainModule.projectRootPath, basePath),
          );
    const sdkPaths: string[] = [];
    for (const basePath of basePaths) {
      for (const directory of ['arkts', 'api', 'kits', 'component']) {
        const sdkPath = path.resolve(basePath, directory);
  ...
  private createDependency(sdkPath: string, filePath: string, prefix: string): readonly [string, DependencyItem] {
