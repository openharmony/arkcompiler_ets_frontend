 ...
 *
 * Accepts `fs::path`, `std::string`, `std::string_view`,
 * `const char *` and similar standard string-like inputs. Implemented as a
 * single function template (rather than overloads on `fs::path`
 ...
 *
 * Uses `WeaklyCanonical` (rather than `canonical`) so this
...
{
    fs::path path;
    if constexpr (std::is_same_v<std::decay_t<PathLike>, fs::path>) {
        path = pathLike;
    } else {
        path = fs::path(std::string_view(pathLike));
    }

    fs::path absolutePath = path;
    try {
        absolutePath = WeaklyCanonical(AbsolutePath(path));
    } catch (const fs::filesystem_error &) {
        // Best-effort normalization: keep the pre-try fallback (the raw input path) instead of
        // propagating the error, since this is used for cosmetic/logging purposes only.
    }
 ...
 *
 * Accepts the same path-like inputs as `NormalizePath` (`fs::path`, `std::string`,
 ...
 * Win32 file-API calls (which every standard file operation -- fstream, fs -- goes
...
/**
 * @brief convert a `fs::file_time_type` (e.g. from
