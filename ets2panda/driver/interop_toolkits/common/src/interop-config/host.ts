...
import { type Language, getLanguageFromSourceCode } from '../fileUtils';
...
export interface InteropConfigHost {
   ...
   */
  getLanguageFromSourceCode(filePath: string): Promise<Language>;
}

/** Creates the default host that custom hosts are merged over. */
export function createDefaultInteropConfigHost(): InteropConfigHost {
  return {
    async getLanguageFromSourceCode(filePath: string): Promise<Language> {
      const sourceCode = await fs.readFile(filePath, 'utf8');
      return getLanguageFromSourceCode(sourceCode);
    },
  };
}
