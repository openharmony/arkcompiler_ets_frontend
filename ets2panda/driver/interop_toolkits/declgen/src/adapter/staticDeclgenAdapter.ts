...
import { StaticDeclgen } from '../static/declgen';
import * as common from '@interop-toolkits/common';

export class StaticDeclgenAdapter extends DeclgenAdapter {
  ...
  async run(entryFiles: string[]): Promise<void> {
    const staticDeclgen = new StaticDeclgen(this.buildConfig, entryFiles, this.compilerConfigPath);
    await staticDeclgen.run();
  }
}
