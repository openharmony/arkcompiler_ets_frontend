...
export function defaultCompilerOptions(): ts.CompilerOptions {
  return {
    ...
    allowJs: true,
    checkJs: true,
    // Without this, TS auto-includes every package under node_modules/@types
    // (jest, babel, istanbul, ...) as global ambient context on every
    // ts.createProgram() call, tripling compile time. 'node' is the only
    // ambient global module declgen's pipeline relies on.
    types: ['node'],
  };
}
...
export class Compiler {
  ...
  private computeCommonSourceDirectory(rootNames: readonly string[]): string {
    ...
    for (const fileName of rootNames) {
      ...
      let i = 0;
      for (; i < n; i++) {
        // Indices are within [0, n) of both arrays, so both accesses are in-bounds.
  ...
  getResolvedImports(file: ts.SourceFile): string[] {
    ...
    if (resolved && typeof resolved.forEach === 'function') {
      resolved.forEach((mod) => collectResolvedImport(mod, out, seen));
    }
    return out;
  }
...
/** Records a single resolved module's absolute path, deduplicated via `seen`. */
function collectResolvedImport(mod: ts.ResolvedModuleFull | undefined, out: string[], seen: Set<string>): void {
