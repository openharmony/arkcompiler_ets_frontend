...
import type { CacheSession, EmittedArtifactInfo } from '../../cache/cacheStore';
...
export class InitialAffectedAnalysisStage extends Stage<undefined, undefined> {
  ...
  execute(ctx: StageContext<undefined>): StageContext<undefined> {
    ...
    const sourceFileByPath = collectSourceFiles(compiler.program, ctx.transformTargetInfos.keys());

    // 1) Compute fileKey for each root, compare to cache.
    const { affected, mustEmit } = this.computeCacheAffected(compiler, cacheSession, sourceFileByPath);

    // 2) Interop-sig closure: any file whose interop signature changed forces
    //    its reverse-import closure into affected.
    const closure = this.expandInteropClosure(ctx, cacheSession, sourceFileByPath, affected);
  ...
  /**
   * Step 1: computes the per-root affected set by diffing each root's current
   * cache-key inputs against the cached entry, plus tsc's own affected set and
   * the on-disk artifact checks. Returns both the plain `affected` set and the
   * `mustEmit` subset (outputs missing/tampered) that forces the full pipeline.
   */
  private computeCacheAffected(
    compiler: Compiler,
    cacheSession: CacheSession,
    sourceFileByPath: Map<string, ts.SourceFile>,
  ): { affected: Set<string>; mustEmit: Set<string> } {
    ...
    for (const [absPath, sf] of sourceFileByPath) {
      if (this.isCacheEntryAffected(compiler, cacheSession, tscAffected, absPath, sf, mustEmit)) {
        affected.add(absPath);
      }
    }
    return { affected, mustEmit };
  }

  /**
   * Whether a single root's cached entry is stale or invalid and the file must
   * be re-processed. Covers: first run / no prior entry, fileKey mismatch,
   * interop-signature mismatch, tsc marking the file affected, a missing
   * declgen-internal cache artifact, and a deleted/tampered user-visible
   * `.d.ets` (the latter also records the file into `mustEmit`).
   */
  private isCacheEntryAffected(
    compiler: Compiler,
    cacheSession: CacheSession,
    tscAffected: ReadonlySet<string> | undefined,
    absPath: string,
    sf: ts.SourceFile,
    mustEmit: Set<string>,
  ): boolean {
    ...
    if ((cached.interopSigHash ?? null) !== interopSigHash) {
      return true;
    }
    if (tscAffected?.has(absPath)) {
      return true;
    }
    ...
    if (!this.outputArtifactsPresent(cached)) {
      return true;
    }
    ...
    if (this.verifyOutputs && !this.emittedArtifactStillValid(cached.emittedArtifact)) {
  ...
  /**
   * Step 2: seeds the affected set with files whose interop signature changed
   * (interop tags change exported function signatures from declgen's POV;
   * importers must rebuild), closes it over the reverse-import graph, and
   * publishes that graph on `ctx` for reuse by downstream
   * AffectedAnalysisStages.
   */
  private expandInteropClosure(
    ...
    const reverseImporters = buildReverseImportGraph(sourceFileByPath, ctx.compiler);
    ...
    return computeReverseClosure(seed, reverseImporters);
  }
...
function collectSourceFiles(program: ts.Program, rootFiles: Iterable<string>): Map<string, ts.SourceFile> {
