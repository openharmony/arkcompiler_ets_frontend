...
class OverloadFixTraverser extends VisitorTraverser<PrevState, LocalState> {
  ...
  protected override buildVisitors(): Map<ts.SyntaxKind, ts.Visitor[]> {
...
function mergeIndexSignatures(
  ...
  const first = sigs[0];
  const mergedKeyParam = mergeIndexKeyParameter(factory, sigs, typeChecker);

  // Merge value type using existing return-type merging logic.
  const mergedValueType = mergeReturnTypes(factory, sigs);

  const mergedModifiers = mergeIndexModifiers(factory, sigs);
...
/**
 * Merge the key parameters of all index signatures into one:
 * - Type: dedup by assembly type, preserve original TypeNodes, union when
 *   several remain (falls back to `string` when no signature carries a type).
 * - Name: kept when all signatures agree, otherwise 'key'.
 */
function mergeIndexKeyParameter(
  factory: ts.NodeFactory,
  sigs: ts.IndexSignatureDeclaration[],
  typeChecker: ts.TypeChecker,
): ts.ParameterDeclaration {
  ...
  const firstKeyParam = sigs[0].parameters[0];
  ...
  return factory.createParameterDeclaration(undefined, undefined, mergedKeyName, undefined, mergedKeyType, undefined);
}

/** readonly: scheme B — only kept when ALL signatures are readonly; other modifiers come from the first signature. */
function mergeIndexModifiers(factory: ts.NodeFactory, sigs: ts.IndexSignatureDeclaration[]): ts.Modifier[] {
  const allReadonly = sigs.every((s) =>
    (ts.getModifiers(s) ?? []).some((m) => m.kind === ts.SyntaxKind.ReadonlyKeyword),
  );
  const firstModifiers = ts.getModifiers(sigs[0]) ?? [];
