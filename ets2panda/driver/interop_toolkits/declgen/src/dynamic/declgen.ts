...
export class Declgen {
  ...
  constructor(
    ...
    this.features = resolveFeatures(declgenOptions.features);
    this.declgenVersion = readDeclgenVersion();

    const cacheEnabled = isCacheEnabled(declgenOptions);
    ...
    this.compilerOptions = buildCompilerOptions(declgenOptions, compilerOptions, cacheEnabled, tsBuildInfoFile);
    this.compiler = new Compiler(this.rootFiles, this.libFiles, this.compilerOptions, customResolveModuleNames);

    this.pipeline = buildPipeline(this.features, declgenOptions.verifyOutputs ?? true);
   ...
   * the whole `StageContext` to keep the exposed API narrow.
   *
  ...
  private buildEmit(
    ...
    return (writeFile?: DeclgenWriteFile): void => {
      const perFileWriter = writeFile ? emitWithWriteFile(compiler, writeFile) : undefined;
      compiler.emit(perFileWriter, recordEmitted, filesToEmit);
   ...
   */
  private buildFilesToEmit(affectedFiles: ReadonlySet<string>, session: CacheSession | undefined): Set<string> {
...
function resolveFeatures(features?: DeclgenFeatureOptions): Required<DeclgenFeatureOptions> {
...
/** Cache is on unless explicitly disabled and incremental or an explicit cache dir is requested. */
function isCacheEnabled(declgenOptions: DeclgenOptions): boolean {
  const noCache = declgenOptions.noCache || process.env.DECLGEN_NO_CACHE === '1';
  return !noCache && (declgenOptions.incremental || !!declgenOptions.cacheDir);
}
...
function buildCompilerOptions(
  ...
  const options = Object.assign({}, defaultCompilerOptions(), {
    declaration: true,
...
function buildPipeline(
  ...
  verifyOutputs: boolean,
): Pipeline<IStage<unknown, unknown>[]> {
  const pipeline = new Pipeline();
  pipeline.add(new CompileStage());
  pipeline.add(new InitialAffectedAnalysisStage({ verifyOutputs }));
  pipeline.add(new CacheLoadStage('declaration'));
  pipeline.add(new DeclarationConversionStage());
  pipeline.add(new CacheSaveStage('declaration'));
  pipeline.add(new InteropTagsStage());
  pipeline.add(new AffectedAnalysisStage('interopTags'));
  pipeline.add(new RecheckStage());
  pipeline.add(new ConversionStage(features.removeReservedKeywordIdentifier));
  pipeline.add(new AffectedAnalysisStage('conversion'));
  pipeline.add(new RecheckStage());
  pipeline.add(new SemanticFixStage());
  pipeline.add(new AffectedAnalysisStage('semanticFix'));
  if (features.enableInteropTypesFix) {
    pipeline.add(new InteropTypesConversionStage());
    pipeline.add(new AffectedAnalysisStage('interopTypes'));
  }
  pipeline.add(new ASTFixStage());
  pipeline.add(new AffectedAnalysisStage('astFix'));
  return pipeline;
}
...
/**
 * Adapts a caller-supplied {@link DeclgenWriteFile} callback into the
 * per-file writer `Compiler.emit` expects: renders the final `.d.ets` text,
 * invokes the callback, and opts into artifact tracking when the callback
 * reports an `artifactPath`.
 */
function emitWithWriteFile(
  compiler: Compiler,
  writeFile: DeclgenWriteFile,
): (fileName: string, sourceFile: ts.SourceFile) => void | { artifactPath?: string; finalContent?: string } {
