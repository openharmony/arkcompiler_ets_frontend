...
function parseCLI(): RunnerCLIOptions {
  ...
  program
    ...
    .option('--test-out-dir <path>', 'Directory where Declgen output is stored. Defaults to a temp directory')
    .option('-j, --jobs <count>', 'Maximum parallel workers', String(Math.floor(os.cpus().length / 2)));
  ...
  return {
    ...
    testOutDir: opts.testOutDir ?? fs.mkdtempSync(path.join(os.tmpdir(), 'declgen-cookbook-')),
...
async function main(): Promise<void> {
  ...
  if (!fs.existsSync(workerScript)) {
    console.error(`Cookbook worker script not found: ${workerScript}`);
    process.exit(1);
  }
  ...
  console.log(
    `Found ${testCases.length} test case(s) across suite(s): [${[...new Set(testCases.map((t) => t.suite))].join(
      ', ',
    )}]`,
  );
  console.log('');
