...
export class CookbookLoader {
  ...
  private loadSuite(suite: string): TestCase[] {
    ...
    const entries = fs.readdirSync(suitePath).filter((e) => {
      ...
      return (suite === 'package_source' && fs.statSync(fullPath).isDirectory()) || fs.statSync(fullPath).isFile();
    });
  ...
  private buildTestCase(
    ...
    const expectedOutput =
      suite === 'package_source' ? path.join(detsDir, name) : path.join(detsDir, `${name}${Extension.DETS}`);
