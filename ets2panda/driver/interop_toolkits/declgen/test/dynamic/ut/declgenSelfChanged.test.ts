 ...
 */
describe('incremental cross-read: abstract method override is synthesized after dep change', () => {
  ...
  it('B.d.ets gains the synthesized override on the second run when A becomes abstract', () => {
    ...
    expect(!/\bfoo\s*\(\s*\)\s*:\s*void/.test(bV1)).toBeTruthy();
    ...
    expect(bV2).toMatch(/foo\s*\(\s*\)\s*:\s*void/);
  });
});
