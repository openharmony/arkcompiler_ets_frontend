...
import { Stage, StageContext, IStage } from '../../../src/dynamic/stages/Stage';
import { DeclgenPlugin, PluginStageSpec } from '../../../src/dynamic/plugin/plugin';
...
describe('plugin system', () => {
  ...
  it('rejects invalid plugin id', () => {
    ...
    expect(() => new Declgen(buildOptions({ plugins: [bad] }))).toThrow(/must match/);
  });
  ...
  it('rejects duplicate plugin ids', () => {
    ...
    expect(() => new Declgen(buildOptions({ plugins: [p1, p2] }))).toThrow(/duplicate stage id/);
  });
  ...
  it('rejects unknown extension point', () => {
    ...
    expect(() => new Declgen(buildOptions({ plugins: [p] }))).toThrow(/not available in the current pipeline/);
  });
  ...
  it('inserts plugin stage at the requested anchor and runs it', () => {
    ...
    expect(seen.has('A.ts')).toBeTruthy();
    expect(seen.has('B.ts')).toBeTruthy();
  });
  ...
  it('cross-read plugin gets an AffectedAnalysisStage(plugin:<id>) appended', () => {
    ...
    expect(observer2.observations.length).toBe(0);
  });
  ...
  it('global plugin sees every root file after a no-change second run', () => {
    ...
    expect(seen.has('A.ts') && seen.has('B.ts')).toBeTruthy();
  });
  ...
  it('plugin config change invalidates the cache', () => {
    ...
    expect(manifest1.globalKey).not.toBe(manifest2.globalKey);
  });
