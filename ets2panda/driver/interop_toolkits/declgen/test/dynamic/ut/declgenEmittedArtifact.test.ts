...
import { Declgen, DeclgenOptions } from '../../../src/dynamic/declgen';
import { Logger } from '../../../src/dynamic/logger/logger';
import { SilentLogger } from '../../../src/dynamic/logger/silentLogger';
...
describe('Declgen emittedArtifact tracking (Phase 2/3)', () => {
  ...
  it('records emittedArtifact for the default emit path', () => {
    ...
    expect(fs.existsSync(layout.detsFile)).toBe(true);
    ...
    expect(emitted!.path).toBe(layout.detsFile);
    expect(typeof emitted!.sha256).toBe('string');
    expect((emitted!.sha256 as string).length === 64).toBeTruthy();
    expect(typeof emitted!.size).toBe('number');
    expect(typeof emitted!.mtime).toBe('number');
  });
  ...
  it('re-emits a file when its previously-recorded .d.ets is deleted', () => {
    ...
    expect(fs.existsSync(layout.detsFile)).toBe(false);
  ...
  it('honors the custom writeFile opt-in via { artifactPath }', () => {
    ...
    expect(write.fileName).toBe(layout.entryFile);
    expect(write.content.startsWith("'use static'\n")).toBeTruthy();

    const entry = readManifestEntry(layout.cacheDir, layout.entryFile);
    const emitted = entry?.emittedArtifact as Record<string, unknown> | undefined;
    expect(emitted).toBeTruthy();
    expect(emitted!.path).toBe(customPath);
  });
  ...
  it('skips emittedArtifact recording when custom writeFile returns undefined', () => {
    ...
    const entry = readManifestEntry(layout.cacheDir, layout.entryFile);
    expect(entry?.emittedArtifact).toBe(undefined);
  });
  ...
  it('passes meta.hash equal to the on-disk file hash and the manifest entry', () => {
    ...
    expect(/^[0-9a-f]{64}$/.test(hash)).toBeTruthy();
    ...
    expect(hash).toBe(diskHash);

    // And the value declgen recorded into the manifest.
    const entry = readManifestEntry(layout.cacheDir, layout.entryFile);
    const emitted = entry?.emittedArtifact as Record<string, unknown> | undefined;
    expect(emitted).toBeTruthy();
    expect(emitted!.sha256).toBe(hash);
  });
  ...
  describe('re-emit after artifact deletion must match a clean full build', () => {
    ...
    it('produces byte-identical outputs whether built fresh or after deleting both .d.ets', () => {
      ...
      expect(baseline.a).toMatch(/some\?\s*:\s*Any/);
      expect(baseline.b).toMatch(/foo\s*\(\s*\)\s*:\s*void/);
      ...
      expect(aReemit).toBe(baseline.a);
      expect(bReemit).toBe(baseline.b);
    });
  });
});
 ...
 */
describe('incremental emit only writes files whose output changed (Scheme B)', () => {
  ...
  beforeAll(() => Logger.init(new SilentLogger()));
  ...
  it('emits the changed dependency but skips an unchanged importer', () => {
    ...
    expect(fs.existsSync(aDets)).toBe(true);
    expect(fs.existsSync(bDets)).toBe(true);
    ...
    expect(emitted.includes(aFile)).toBeTruthy();
    expect(!emitted.includes(bFile)).toBeTruthy();

    // The changed dependency picked up its new export.
    expect(fs.readFileSync(aDets, 'utf8')).toMatch(/helper/);
    // The skipped importer is untouched and still has its v1 content.
    expect(fs.readFileSync(bDets, 'utf8')).toBe(bV1);
  });
  ...
  it('no-verify-outputs: deleted outputs are not regenerated until the source changes', () => {
    ...
    expect(run2Emitted.length).toBe(0);
    expect(fs.existsSync(aDets)).toBe(false);
    expect(fs.existsSync(bDets)).toBe(false);
    ...
    expect(run3Emitted.includes(aFile)).toBeTruthy();
    expect(run3Emitted.includes(bFile)).toBeTruthy();
    expect(fs.readFileSync(bDets, 'utf8')).toMatch(/foo\s*\(\s*\)\s*:\s*void/);
  });
});
