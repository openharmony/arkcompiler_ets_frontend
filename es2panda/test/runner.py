...
def get_args():
    ...
    parser.add_argument(
        '--jobs', '-j', type=int, dest='jobs', default=None,
        help='number of parallel jobs: thread pool of the version-control '
             'suites (default: min(cpu, 16); --jobs=1 forces serial '
             'execution) and process pool size of the other suites '
             '(default: cpu count)')
    parser.add_argument(
        '--vc-versions', dest='vc_versions', default=None,
        help='comma-separated API versions to run for version-control tests, e.g. '
             'API12beta3 or API12,API18 (API12 matches all its betas); '
             'restricts compile-target and VM iteration lists for debugging')
    parser.add_argument(
        '--vc-phase', dest='vc_phase', default=None,
        help='comma-separated version-control phases to run for debugging: '
             'source (source-level compile + VM matrix), abc (abc-input '
             'recompile + VM), es2abc (recompile inputs generated by old '
             'es2abc versions, compile-only); default: all three')
    parser.add_argument(
    ...
    args.vc_version_specs = None
    if args.vc_versions:
        valid_versions = set(API_VERSION_MAP.keys()) | {"API12"}
        specs = {spec.strip() for spec in args.vc_versions.split(",") if spec.strip()}
        invalid = specs - valid_versions
        if invalid:
            parser.error(
                "Invalid --vc-versions value(s): %s. Valid values: %s"
                % (",".join(sorted(invalid)), ",".join(sorted(valid_versions)))
            )
        args.vc_version_specs = specs
    args.vc_phase_set = None
    if args.vc_phase:
        valid_phases = {"source", "abc", "es2abc"}
        selected_phases = {p.strip() for p in args.vc_phase.split(",") if p.strip()}
        invalid_phases = selected_phases - valid_phases
        if invalid_phases:
            parser.error(
                "Invalid --vc-phase value(s): %s. Valid values: %s"
                % (",".join(sorted(invalid_phases)), ",".join(sorted(valid_phases)))
            )
        args.vc_phase_set = selected_phases
    return args
...
class FailedToStartProcess:
    # Stand-in returned by run_with_timeout when the command cannot even be
    # started (missing or non-executable binary). Every comparison site
    # treats a non-zero returncode plus non-empty stderr as a failure, so a
    # crashed launch degrades into an ordinary test failure instead of an
    # unpicklable traceback that kills the whole run.
    returncode = 127


def run_with_timeout(cmd, timeout=None, cwd=None, env=None):
    try:
        process = subprocess.Popen(
            cmd, cwd=cwd, env=env, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    except OSError as exc:
        message = "Failed to start command: %s\n%s: %s" % (cmd, type(exc).__name__, exc)
        return FailedToStartProcess(), b"", message.encode("utf-8")
    try:
        stdout, stderr = process.communicate(timeout=timeout)
    except subprocess.TimeoutExpired:
        process.kill()
        stdout, stderr = process.communicate()
        stderr = ("Timeout after %ss: %s" % (timeout, cmd)).encode("utf-8")
    return process, stdout, stderr


def vc_version_selected(name, args):
    # "API12" as a family spec matches API12beta1/2/3; exact specs match
    # themselves. The prefix rule can only stay this simple because no two
    # distinct names of API_VERSION_MAP/ARK_JS_VM_LIST are prefixes of each
    # other apart from the intended API12 family; keep that invariant when
    # adding versions to config.py.
    specs = getattr(args, "vc_version_specs", None)
    if not specs:
        return True
    for spec in specs:
        if spec == name or ("beta" not in spec and name.startswith(spec)):
            return True
    return False


def default_jobs(args):
    return args.jobs if args.jobs else min(os.cpu_count() or 4, 16)


def expected_mismatch_note(expected_path):
    # Comparison failures with an otherwise-empty stderr used to report
    # nothing, hiding which expected file the fallback chain had picked;
    # always name the file that was actually consulted.
    return "Comparison failed against expected file: %s" % expected_path


def vc_vm_timeout(args):
    # Version-control VM runs never had a runtime timeout before; the slowest
    # legitimate run (wide lazy-import, measured ~12s) exceeds the default
    # --timeout of 10s, so the timeout here is only a hang guard: never below
    # 120s, but an explicitly larger --timeout is honored.
    return max(args.timeout, 120)


def vc_flattened_name(file_path):
    # Flattened artifact names are relative to the es2panda root instead of
    # absolute: old ark_js_vm builds are unreliable with very long abc paths,
    # and the absolute-path flattening produced ~370+ char paths.
    es2panda_root = path.dirname(path.dirname(path.abspath(__file__)))
    return os.path.relpath(file_path, es2panda_root).replace("/", "_")


# log_cmd is called from the version-control thread pools and must be
# serialized, but Test instances also cross process boundaries via
# multiprocessing.Pool for the other suites, and a per-instance
# threading.Lock is not picklable -- so the lock stays module-level.
LOG_CMD_LOCK = threading.Lock()
...
class Runner:
    ...
    def run(self):
        pool = multiprocessing.Pool(processes=self.args.jobs)
    ...
    def run_threaded(self):
        jobs = default_jobs(self.args)
        jobs = max(1, min(jobs, len(self.tests))) if self.tests else 1
        note = ""
        if getattr(self.args, "vc_version_specs", None):
            note = ", versions: %s (filtered)" % ",".join(sorted(self.args.vc_version_specs))
        print("[%s] %d tests, jobs=%d%s" % (self.name, len(self.tests), jobs, note))
        if jobs == 1:
            self.tests = [self.run_test(t) for t in self.tests]
            return
        with concurrent.futures.ThreadPoolExecutor(max_workers=jobs) as pool:
            futures = {pool.submit(self.run_test, t): i for i, t in enumerate(self.tests)}
            done_iter = concurrent.futures.as_completed(futures)
            if self.args.progress:
                from tqdm import tqdm
                done_iter = tqdm(done_iter, total=len(futures))
            results = [None] * len(self.tests)
...
class AbcTestCasesPrepare:
    ...
    def add_abc_directory(self, directory, extension):
        ...
        # Deliberately not filtered by --filter: the generated inputs are
        # dependencies of the test cases (e.g. mixed-compile configs reference
        # them by literal path), so the prepared set must stay complete.
        return glob(glob_expression)
    ...
    def gen_abc_tests(self, directory, extension, flags, abc_mode, es2abc_versions=["default"]):
        ...
        for es2abc_version in es2abc_versions:
            for input_path in test_source_list:
                for api_version in ES2ABC_API_SUPPORT.get(es2abc_version):
                    main_version, beta_version = AbcTestCasesPrepare.split_api_version(api_version)
                    output_path = self.get_output_path(input_path, main_version, beta_version, es2abc_version)
                    self.test_abc_path_list.add(output_path)
                    compile_jobs.append(
                        (flags, input_path, output_path, main_version, beta_version, es2abc_version)
                    )
        if not compile_jobs:
            return

        def run_compile_job(job):
            flags, input_path, output_path, main_version, beta_version, es2abc_version = job
            return self.compile_for_target_version(
                flags, input_path, output_path, main_version, beta_version, es2abc_version
            )

        jobs = default_jobs(self.args)
        first_error = None
        with concurrent.futures.ThreadPoolExecutor(max_workers=max(1, min(jobs, len(compile_jobs)))) as pool:
            for _, stderr in pool.map(run_compile_job, compile_jobs):
                if stderr and first_error is None:
                    first_error = stderr
        if first_error is not None:
            raise RuntimeError(f"abc generate error: {first_error}")
    ...
    def compile_for_target_version(self, flags, input_path, output_path, target_api_version, target_api_sub_version="", es2abc_version="default"):
        ...
        process, stdout, stderr = run_with_timeout(cmd, self.args.es2panda_timeout)
        if stderr:
...
class AbcVersionControlRunner(Runner):
    ...
    def run(self):
        # Shared pool for the per-test VM matrices (same rationale as
        # VersionControlRunner); it mostly matters for --filter runs where
        # only a few tests exist to spread across run_threaded's workers.
        jobs = default_jobs(self.args)
        if jobs > 1:
            self.vm_pool = concurrent.futures.ThreadPoolExecutor(max_workers=jobs * 2)
        try:
            self.run_threaded()
        finally:
            if getattr(self, "vm_pool", None) is not None:
                self.vm_pool.shutdown()
                self.vm_pool = None
...
class VersionControlRunner(Runner):
    ...
    def add_module_path(self, module_dir):
        # Modules are inputs of the test cases, not test cases themselves:
        # never filtered by --filter, otherwise a single-case run of a
        # module-importing test would compile without its module abc.
    ...
    def run(self):
        # Shared pool for the per-test VM matrices; each of the (up to) 8 VM
        # iterations of a test is independent (private abc directory), so they
        # can run concurrently without affecting comparison semantics.
        jobs = default_jobs(self.args)
        if jobs > 1:
            self.vm_pool = concurrent.futures.ThreadPoolExecutor(max_workers=jobs * 2)
        try:
...
class TestAbcVersionControl(Test):
    ...
    def run_process(self, cmd, timeout=None, env=None):
        self.process, stdout, stderr = run_with_timeout(cmd, timeout, env=env)
    ...
    def compile_for_target_version(
        ...
        if target_api_sub_version != "":
            cmd.append("--target-api-sub-version=%s" % (target_api_sub_version))
        self.log_cmd(cmd)
        stdout, stderr = self.run_process(cmd, timeout=runner.args.es2panda_timeout)
        return stdout, stderr

    def build_abc_filename(self, target_api_version, target_api_sub_version):
        prefix = vc_flattened_name(path.splitext(self.path)[0])
    ...
    def generate_abc(self, runner, target_api_version, target_api_sub_version=""):
        ...
        output_root = path.join(runner.build_dir, "vc_work")
        os.makedirs(output_root, exist_ok=True)
        self.target_abc_path = path.join(output_root, target_abc_name)
        ...
        try:
            ...
            self.passed = False
        if not self.passed and not stderr:
            stderr = expected_mismatch_note(compile_expected_path)
        return stderr
    ...
    def execute_abc(self, runner, vm_version, entry_point=""):
        ...
        self.log_cmd(cmd)
        process, stdout, stderr = run_with_timeout(cmd, vc_vm_timeout(runner.args), env=env)
        output = stdout.decode("utf-8", errors="ignore") + stderr.decode("utf-8", errors="ignore").split("\n")[0]
        if stderr:
            stderr = "Error executing command: %s\n%s" % (cmd, stderr)
        return stderr, process, output

    def test_abc_execution(self, runner, target_api_version, target_api_sub_version=""):
        target_version = "API" + target_api_version + target_api_sub_version
        target_version_number = API_VERSION_MAP.get(target_version)
        vm_jobs = [
            vm_version for vm_version in sorted(ARK_JS_VM_LIST)
            if vc_version_selected(vm_version, runner.args)
        ]

        def execute_vm_job(vm_version):
            # VM iterations are independent (each runs the already-compiled
            # target abc), so they can execute concurrently; the shared
            # instance state is applied afterwards in iteration order.
            stderr, process, output = self.execute_abc(runner, vm_version, self.entry_point)
            ...
            runtime_expect_path = self.get_path_to_expected(is_support, "runtime")
            try:
                with open(runtime_expect_path, "r") as fp:
                    expected = fp.read()
                if is_support and self.abc_mode != "merge_mode":
                    passed = expected == output and process.returncode in [0, 1, 255]
                else:
                    passed = expected in output
            except Exception:
                passed = False
            if not passed and not stderr:
                stderr = expected_mismatch_note(runtime_expect_path)
            return {"stderr": stderr, "process": process, "output": output,
                    "passed": passed, "is_support": is_support}

        vm_pool = getattr(runner, "vm_pool", None)
        if vm_pool is not None and len(vm_jobs) > 1:
            futures = [vm_pool.submit(execute_vm_job, vm_version) for vm_version in vm_jobs]
            results = [future.result() for future in futures]
        else:
            results = [execute_vm_job(vm_version) for vm_version in vm_jobs]

        # Apply results in iteration order so the verdict and the reported
        # error match the serial fail-fast behavior.
        stderr = None
        for result in results:
            self.is_support = result["is_support"]
    ...
    def run(self, runner):
        for api_version in API_VERSION_MAP:
            if not vc_version_selected(api_version, runner.args):
                continue
...
class TestEs2abcVersionControl(TestAbcVersionControl):
    ...
    @classmethod
    def should_skip_abc_file(cls, input_version_numbers, es2abc_version: str) -> bool:
        # input_version_numbers is the ascending-sorted list of the input's
        # version numbers (see extract_input_versions); the newest member
        # decides whether this es2abc can read the file at all.
        max_input_version = input_version_numbers[-1]
        max_supported_version = cls.get_max_supported_version(es2abc_version)
        return cls.version_str_to_tuple(max_input_version) > cls.version_str_to_tuple(max_supported_version)
    ...
    def compile_for_target_version(
        ...
        cmd = []
        es2panda_path = (
            runner.es2panda if es2abc_version == "default"
            else os.path.join(runner.build_dir, "es2abc_version", es2abc_version, "es2abc")
        )
        cmd.append(es2panda_path)
        cmd.append(input_path)
        cmd.extend(self.flags)
    ...
    def expected_compile_output_matches(self, compile_expected_path, format_content):
        try:
            with open(compile_expected_path, "r") as fp:
                expected = fp.read()
            return (
                expected.format(format_content) in self.output
                and self.process.returncode in [0, 1]
            )
        except Exception:
            return False
    ...
    def generate_abc(self, runner, target_api_version, target_api_sub_version=""):
        ...
        # The expected behavior depends only on the target/input version
        # pair, not on the recompiling es2abc, so it is decided once and
        # every es2abc version is checked against the same expectation, in
        # list order with fail-fast on the first failing version.
        is_support, compile_expected_path, format_content = self.determine_expected_behavior(
            target_api_version, target_api_sub_version, input_version_numbers
        )
        self.is_support = is_support
        ...
        for es2abc_version in self.es2abc_versions:
            # The version of the abc file exceeds the maximum supported range
            # of this es2abc: recompilation is out of scope for it.
            ...
            _, version_stderr = self.compile_for_target_version(
                runner, self.path, self.target_abc_path, target_api_version, target_api_sub_version, es2abc_version
            )
            if self.is_support:
                self.passed = not version_stderr
            else:
                self.passed = self.expected_compile_output_matches(compile_expected_path, format_content)
            if not self.passed:
                return version_stderr if version_stderr else expected_mismatch_note(compile_expected_path)
...
class VersionControlPathHelper:
    ...
    def run_process_in_es2panda_dir(self, runner, cmd, timeout=None):
        cwd = self.get_es2panda_cwd(runner)
        self.process, stdout, stderr = run_with_timeout(cmd, timeout, cwd=cwd)
...
class TestVersionControl(VersionControlPathHelper, Test):
    ...
    def generate_single_module_abc(self, runner, module_path, target_version, output_dir=None):
        cmd = []
        cmd.append(runner.es2panda)
        ...
        module_abc_dir = path.join(output_dir if output_dir else self.work_dir, relative_path)
        os.makedirs(module_abc_dir, exist_ok=True)
        module_abc_path = path.join(module_abc_dir, module_abc_name)
        cmd.extend(["--output=%s" % (module_abc_path)])
        self.log_cmd(cmd)

        _, _, stderr = run_with_timeout(
            cmd, runner.args.es2panda_timeout, cwd=self.get_es2panda_cwd(runner)
        )
        if stderr:
            return "Module abc compile failed (%s):\n%s" % (
                module_path, stderr.decode("utf-8", errors="ignore")
            ).encode("utf-8")
        return None
    ...
    def run_process(self, cmd, timeout=None):
        self.process, stdout, stderr = run_with_timeout(cmd, timeout)
        self.output = stdout.decode("utf-8", errors="ignore") + stderr.decode("utf-8", errors="ignore")
        return stdout, stderr
    ...
    def run_process_compile(self, runner, target_api_version, target_api_sub_version="bata3", dump_type=""):
        ...
        cmd.extend(self.flags)
        cmd.append("--target-api-version=%s" % (target_api_version))
        self.test_abc_path = path.join(self.work_dir, "main.abc")
        ...
        self.log_cmd(cmd)
        stdout, stderr = self.run_process_in_es2panda_dir(
            runner, cmd, timeout=runner.args.es2panda_timeout
        )
        return stdout, stderr
    ...
    def runtime_for_target_version(self, runner, target_api_version, target_api_sub_version, abc_path):
        cmd = []
        if target_api_version != "12":
        ...
        process, stdout, stderr = run_with_timeout(cmd, vc_vm_timeout(runner.args), env=env)
        output = stdout.decode("utf-8", errors="ignore") + stderr.decode("utf-8", errors="ignore").split("\n")[0]
        return stdout, stderr, process, output
    ...
    def run_for_single_version(self, runner, target_api_version, target_api_sub_version=""):
        ...
        try:
            ...
            self.passed = False
        if not self.passed:
            return stderr if stderr else expected_mismatch_note(compile_expected_path).encode("utf-8")
        if stderr:
            # stderr with a passing comparison means the compile was expected
            # to fail and the golden error text matched (it lives in stderr);
            # the main abc was never produced, so the VM matrix for this
        ...
        for api_version in self.target_api_version_list:
            ...
            for api_sub_version in self.target_api_sub_version_list:
                ...
                if not vc_version_selected(cur_runtime_api_version, runner.args):
                    continue
                ...
                runtime_jobs.append((api_version, api_sub_version, cur_runtime_api_version, is_below_abc_version))

        def execute_runtime_job(job):
            api_version, api_sub_version, cur_runtime_api_version, is_below_abc_version = job
            # Every runtime version runs in its own directory with a copy of
            # the main abc and its own module abcs (their content depends on
            # the runtime version), keeping iterations independent so the VM
            # matrix of one test case can execute concurrently.
            runtime_dir = path.join(self.work_dir, "runtime_%s" % cur_runtime_api_version)
            os.makedirs(runtime_dir, exist_ok=True)
            runtime_abc_path = path.join(runtime_dir, "main.abc")
            shutil.copyfile(self.test_abc_path, runtime_abc_path)
            for module_path in self.module_path_list:
                module_stderr = self.generate_single_module_abc(
                    runner, module_path, cur_runtime_api_version, runtime_dir
                )
                if module_stderr:
                    # A module that failed to compile would otherwise only
                    # surface as an unrelated VM output mismatch downstream;
                    # fail this cell directly with the module path.
                    return {"stderr": module_stderr, "process": None, "output": "", "passed": False}
            _, stderr, process, output = self.runtime_for_target_version(
                runner, api_version, api_sub_version, runtime_abc_path
            )
            ...
            try:
                ...
                passed = False
            if not passed and not stderr:
                stderr = expected_mismatch_note(runtime_expected_path).encode("utf-8")
            return {"stderr": stderr, "process": process, "output": output, "passed": passed}

        vm_pool = getattr(runner, "vm_pool", None)
        if vm_pool is not None and len(runtime_jobs) > 1:
            futures = [vm_pool.submit(execute_runtime_job, job) for job in runtime_jobs]
            results = [future.result() for future in futures]
        else:
            results = [execute_runtime_job(job) for job in runtime_jobs]
    ...
    def run(self, runner):
        self.work_dir = path.join(runner.build_dir, "vc_work", vc_flattened_name(self.path))
        ...
        try:
            for target_api_version in self.target_api_version_list:
                ...
                if target_api_version == "12":
                    if not any(
                        vc_version_selected("API12" + sub, runner.args)
                        for sub in self.target_api_sub_version_list
                    ):
                        continue
                    for target_api_sub_version in self.target_api_sub_version_list:
                        if not vc_version_selected("API12" + target_api_sub_version, runner.args):
                            continue
                        stderr = self.run_for_single_version(runner, target_api_version, target_api_sub_version)
                        if self.test_abc_path and path.exists(self.test_abc_path):
                            os.remove(self.test_abc_path)
                        if not self.passed:
                            self.error = stderr.decode("utf-8", errors="ignore")
                            return self
                else:
                    if not vc_version_selected("API" + target_api_version, runner.args):
                        continue
...
# Teardown hooks live in a module-level registry instead of on args:
# the multiprocessing suites (regression/compiler/...) pickle their Runner
# — and with it args — into worker processes, and closures are not
# picklable. This only bites when version-control runs in the same
# invocation as those suites.
VC_TEARDOWN_HOOKS = []


def clean_stale_artifacts(build_dir):
    test_root = path.dirname(path.abspath(__file__))
    stale_patterns = [
        # P1 outputs are written into the source tree; leftovers from an
        # interrupted run would be globbed as extra test cases next time.
        path.join(test_root, "version_control", "bytecode_version_control", "**", "*_version_API*.abc"),
        # Flattened P2/P3/P4 outputs from an interrupted run.
        path.join(build_dir, "*version_control*.abc"),
    ]
    removed = 0
    for pattern in stale_patterns:
        for stale in glob(pattern, recursive=True):
            if path.isfile(stale):
                os.remove(stale)
                removed += 1
    vc_work = path.join(build_dir, "vc_work")
    if path.isdir(vc_work):
        shutil.rmtree(vc_work, ignore_errors=True)
    return removed


def add_directory_for_version_control(runners, args):
    # VM cells that refuse a newer abc exit through SIGABRT, and the abort is
    # itself the asserted behavior; skip core-dump generation for the suite's
    # children. Set in the parent so every spawned process inherits it (this
    # also covers es2abc crashes: their stderr is still captured and shown).
    try:
        resource.setrlimit(resource.RLIMIT_CORE, (0, 0))
    except (ValueError, OSError):
        pass
    phases = getattr(args, "vc_phase_set", None) or {"source", "abc", "es2abc"}
    removed = clean_stale_artifacts(args.build_dir)
    if removed:
        print("[Prepare] cleaned %d stale abc artifacts from previous runs" % removed)
    tools = [
        {
    ...
    # building the test list only globs the sources, so it is cheap to do
    # unconditionally; the runner is appended (and thus run) per selection
    if "source" in phases:
        vc_runners.append(runner)

    abc_tests_prepare = None
    if "abc" in phases or "es2abc" in phases:
        abc_tests_prepare = AbcTestCasesPrepare(args)
        es2abc_versions = list(ES2ABC_API_SUPPORT.keys())

        print("[Prepare] generating version-specific abc inputs ...")
    ...
    if "abc" in phases:
        ...
        vc_runners.append(abc_version_control_runner)
    ...
    if "es2abc" in phases:
        ...
        vc_runners.append(es2abc_version_control_runner)

    # The source-level phase works only from test sources and never touches
    # the generated abc inputs, so it can overlap the (mutually sequential)
    # later phases that share the flattened vc_work namespace. Only that
    # first phase may overlap: the abc phases write identical vc_work names
    # and must stay ordered among themselves.
    if len(vc_runners) > 1 and "source" in phases:
        vc_runners[0].overlaps_next = True
        for vc_runner in vc_runners:
            # with overlap active, defer every VC summary to the end so the
            # reports print in phase order instead of interleaving
            vc_runner.defer_summary = True
    runners.extend(vc_runners)

    def vc_teardown():
        # Runs from run_test_suites after every phase (including an
        # overlapped one) has finished or crashed: drop the inputs generated
        # by the prepare step and any abc left in vc_work by failed tests.
        if abc_tests_prepare is not None:
            abc_tests_prepare.remove_abc_tests()
        shutil.rmtree(path.join(args.build_dir, "vc_work"), ignore_errors=True)

    VC_TEARDOWN_HOOKS.append(vc_teardown)
...
def add_test_suites(runners, args):
    if args.regression:
...
def run_test_suites(args, runners):
    failed_tests = 0
    background = []

    def run_in_background(runner):
        try:
            runner.run()
        except Exception as exc:
            # keep the thread alive; run_test_suites re-raises this after
            # joining
            runner.bg_crash = exc

    try:
        for runner in runners:
            if getattr(runner, "overlaps_next", False):
                thread = threading.Thread(target=run_in_background, args=(runner,))
                thread.start()
                background.append((thread, runner))
                continue
            runner.run()
            if not getattr(runner, "defer_summary", False):
                failed_tests += runner.summarize()
        for thread, runner in background:
            thread.join()
            if getattr(runner, "bg_crash", None) is not None:
                raise runner.bg_crash
        for runner in runners:
            if getattr(runner, "defer_summary", False):
                failed_tests += runner.summarize()
    finally:
        # never tear down while an overlapped phase may still write into
        # vc_work, and run the cleanup even when a phase crashed
        for thread, _ in background:
            thread.join()
        for teardown in VC_TEARDOWN_HOOKS:
            teardown()
    return failed_tests
...
def main():
    ...
    runners = []
    add_test_suites(runners, args)
    failed_tests = run_test_suites(args, runners)
