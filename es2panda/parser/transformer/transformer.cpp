...
ir::UpdateNodes Transformer::VisitClassDeclaration(ir::ClassDeclaration *node)
{
    ...
    if (hasClassDecorators) {
        ...
        // When a sendable class has class decorators, the alias variable generated by the transformer
        // should also be treated as a sendable object and placed in the sendable env. Binding the
        // ClassDefinition node to the alias declaration ensures NeedSetInSendableEnv() returns true.
        ir::AstNode *aliasBindNode = node->Definition()->IsSendable() ? node->Definition() : nullptr;
        res.push_back(CreateVariableDeclarationWithIdentify(aliasName, VariableParsingFlags::VAR,
            node->Definition()->Ident(), false, nullptr, true, aliasBindNode));
                                                                            ...
                                                                            ir::AstNode *bindNode)
{
    ...
    if (needBinding) {
        ...
        decl->BindNode(bindNode != nullptr ? bindNode : declaration);
    }
