    ...
    uint32_t newFileHash, panda::ArenaAllocator *allocator)
{
    ...
    if (protoCache.hashcode() != newFileHash) {
        return nullptr;
    }
    ...
    auto *programCache = allocator->New<panda::es2panda::util::ProgramCache>(newFileHash, std::move(*program));
...
panda::es2panda::util::AbcProgramsCache *ProtobufSnapshotGenerator::GetAbcInputCacheContext(
    const std::string &cacheFilePath, uint32_t newFileHash, panda::ArenaAllocator *allocator)
{
    std::fstream input = panda::es2panda::util::Helpers::FileStream<std::fstream>(
        panda::os::file::File::GetExtendedFilePath(cacheFilePath),
        std::ios::in | std::ios::binary);
    ...
    if (abcProtoCache.hashcode() != newFileHash) {
        return nullptr;
    }
